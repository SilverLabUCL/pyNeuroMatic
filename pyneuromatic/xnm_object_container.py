...
class NMObjectContainer(NMObject):
    ...
    Known Children:
        ...
        NMChannelContainer, NMSetContainer
    ...
    def __eq__(
        ...
        if not super().__eq__(other):
            return False
        if not isinstance(other._NMObjectContainer__nmobject, type(self.__nmobject)):
            return False
    ...
    def _isequivalent(self, other: NMObjectContainerType, alert: bool = False) -> bool:
        if not super()._isequivalent(other, alert=alert):
            return False
        if other.count != self.count:
            if alert:
                a = (
                    "unequivalent container count: "
                    + str(self.count)
                    + " vs "
                    + str(other.count)
                )
    ...
    def _prefix_set(
        self, newprefix: Optional[str] = None, quiet: bool = nmp.QUIET
    ) -> bool:
    ...
    def index(self, name: str) -> int:  # special: 'select'
    ...
    def new(
        self, name: str = "default", select: bool = True, quiet: bool = nmp.QUIET
    ) -> NMObjectType:
        """
        ...
        if self.exists(name):
            e = self._error(
                self.content_type() + " " + nmu.quotes(name) + " already exists"
            )
            raise RuntimeError(e)
    ...
    def append(
        self, nmobject: NMObjectType, select: bool = True, quiet: bool = nmp.QUIET
    ) -> bool:
        """
    ...
    def rename(self, name: str, newname: str, quiet: bool = nmp.QUIET) -> str:
        if not self.__rename:
            e = self._error(self.content_type() + " items cannot be renamed")
            raise RuntimeError(e)
        if not isinstance(name, str):
        ...
        if self.exists(newname):
            e = self._error(
                self.content_type() + " " + nmu.quotes(newname) + " already exists"
            )
            raise RuntimeError(e)
    ...
    def duplicate(
        self, name: str, newname: str, select: bool = True, quiet: bool = nmp.QUIET
    ) -> NMObjectType:
        """
        Copy NMObject.
    ...
    def name_next(self, first: int = 0) -> str:
    ...
    def name_next_seq(self, prefix: str = "default", first: int = 0) -> int:
